...
export function useNotifications() {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`https://site--fronted-redibo--pnmmfvfj8jpw.code.run/api/notificaciones/dropdown-notificaciones/${userId}`);
  ...
  const fetchUnreadCount = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`https://site--fronted-redibo--pnmmfvfj8jpw.code.run/api/notificaciones/notificaciones-no-leidas/${userId}`);
      const data = await response.json();
  ...
  const markAsRead = useCallback(async (notificationId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://site--fronted-redibo--pnmmfvfj8jpw.code.run/api/notificaciones/notificacion-leida/${notificationId}/${userId}`,
        { method: 'PUT' }
      );
